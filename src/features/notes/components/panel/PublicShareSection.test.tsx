...
function useShareLinkResponse(data: ShareLink | null) {
  server.use(
    http.get(`${NOTION_URL}/api/v1/pages/${PAGE_ID}/share-link`, () => envelope(data)),
  );
}
...
describe('nửa công khai của tab Chia sẻ', () => {
  it('hiện skeleton khi đang tải', () => {
    server.use(
      http.get(`${NOTION_URL}/api/v1/pages/${PAGE_ID}/share-link`, () => new Promise(() => undefined)),
    );
  ...
  it('hiện lỗi nhỏ và cho thử tải lại', async () => {
    ...
    renderWithProviders(<PublicShareSection pageId={PAGE_ID} />);
    expect(await screen.findByText('Không tải được liên kết chia sẻ')).toBeInTheDocument();

    useShareLinkResponse(null);
    await userEvent.click(screen.getByRole('button', { name: 'Thử lại' }));

    expect(await screen.findByRole('button', { name: 'Bật chia sẻ công khai' })).toBeInTheDocument();
  });
  ...
  it('chưa có link nào → hiện nút bật, bấm gửi đúng request và chuyển sang form sửa', async () => {
    ...
    server.use(
      http.get(`${NOTION_URL}/api/v1/pages/${PAGE_ID}/share-link`, () => envelope(currentLink)),
      http.post(`${NOTION_URL}/api/v1/pages/${PAGE_ID}/share-link`, async ({ request }) => {
    ...
    await userEvent.click(await screen.findByRole('button', { name: 'Bật chia sẻ công khai' }));

    await waitFor(() => expect(requestBody).toEqual({}));
    expect(await screen.findByRole('button', { name: 'Thu hồi liên kết' })).toBeInTheDocument();
  });
  ...
  it('link đã bị thu hồi vẫn hiện nút bật lại, không hiện form sửa', async () => {
    ...
    expect(await screen.findByRole('button', { name: 'Bật chia sẻ công khai' })).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Thu hồi liên kết' })).not.toBeInTheDocument();
  });
  ...
  it('link đang hoạt động → hiện đúng URL và copy gọi clipboard', async () => {
    ...
    expect(writeTextSpy).toHaveBeenCalledWith(expect.stringContaining('/p/tok_xyz789'));
  });

  it('đổi tuỳ chọn rồi lưu → PATCH đúng field, không gửi password khi để trống', async () => {
    useShareLinkResponse(buildLink({ includeSubpages: false, allowIndexing: false, showAuthors: false }));
    let requestBody: unknown;
    server.use(
      http.patch(`${NOTION_URL}/api/v1/share-links/link-1`, async ({ request }) => {
        requestBody = await request.json();
    ...
    await user.click(await screen.findByRole('checkbox', { name: 'Bao gồm cả trang con' }));
  ...
  it('tick xoá mật khẩu → PATCH gửi password: null', async () => {
    ...
    await user.click(await screen.findByRole('checkbox', { name: 'Xoá mật khẩu hiện có (nếu có)' }));
    await user.click(screen.getByRole('button', { name: 'Lưu thay đổi' }));
  ...
  it('thu hồi: huỷ dialog không gọi DELETE, xác nhận thì gọi', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: 'Thu hồi liên kết' }));
