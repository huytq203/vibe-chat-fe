...
function buildVersion(input: Partial<PageVersion> & Pick<PageVersion, 'id'>): PageVersion {
...
function useVersionList(versions: PageVersion[]) {
  server.use(
    http.get(`${NOTION_URL}/api/v1/pages/${PAGE_ID}/versions`, () => envelope(versions)),
  );
}

function useVersionDetail(version: PageVersion, html = '<p>Nội dung phiên bản</p>') {
  server.use(
...
describe('lịch sử phiên bản của trang', () => {
  ...
  it('hiện lỗi nhỏ và cho thử tải lại', async () => {
    ...
    renderWithProviders(<VersionList pageId={PAGE_ID} />);
    expect(await screen.findByText('Không tải được lịch sử phiên bản')).toBeInTheDocument();

    useVersionList([]);
    await userEvent.click(screen.getByRole('button', { name: 'Thử lại' }));

    expect(await screen.findByText('Chưa có phiên bản nào')).toBeInTheDocument();
  });
  ...
  it('hiện trạng thái rỗng và vẫn cho tạo mốc', async () => {
    ...
    expect(screen.getByRole('textbox', { name: 'Nhãn mốc phiên bản' })).not.toBeRequired();
  });
  ...
  it('nhóm hai mốc cùng ngày và hiện đủ ba nhãn loại tiếng Việt', async () => {
    const versions = [
      ...
      buildVersion({
        id: 'version-manual', kind: 'MANUAL', label: 'Trước khi sửa',
        createdAt: '2026-08-24T05:30:00.000Z',
      }),
      buildVersion({
        id: 'version-restore', kind: 'BEFORE_RESTORE',
    ...
    const sameDayGroup = headings.find((heading) => heading.textContent?.includes('24'));
    expect(sameDayGroup).toBeDefined();
    expect(within(sameDayGroup?.closest('section') as HTMLElement).getAllByRole('button')).toHaveLength(2);
  });
  ...
  it('gửi nhãn người dùng khi tạo mốc thủ công', async () => {
    ...
    useVersionList([]);
    server.use(
      http.post(`${NOTION_URL}/api/v1/pages/${PAGE_ID}/versions`, async ({ request }) => {
        requestBody = await request.json();
        return envelope(buildVersion({ id: 'version-new', kind: 'MANUAL', label: 'Bản chốt' }));
      }),
    );
    renderWithProviders(<VersionList pageId={PAGE_ID} />);
    const user = userEvent.setup();

    await user.type(screen.getByRole('textbox', { name: 'Nhãn mốc phiên bản' }), '  Bản chốt  ');
    await user.click(screen.getByRole('button', { name: /Tạo mốc/ }));

    await waitFor(() => expect(requestBody).toEqual({ label: 'Bản chốt' }));
    expect(screen.getByRole('textbox', { name: 'Nhãn mốc phiên bản' })).toHaveValue('');
  });

  it('cô lập HTML lạ trong iframe sandbox thay vì DOM chính', async () => {
    const version = buildVersion({ id: 'version-risky' });
    const html = '<h1>Nội dung lạ</h1><script>document.body.dataset.xss="true"</script>';
    useVersionList([version]);
    useVersionDetail(version, html);
    renderWithProviders(<VersionList pageId={PAGE_ID} />);

    await userEvent.click(await screen.findByRole('button', { name: /Xem phiên bản/ }));
  ...
  it('chỉ khôi phục sau xác nhận và huỷ dialog không gọi mutation', async () => {
    ...
    server.use(
      http.post(`${NOTION_URL}/api/v1/versions/${version.id}/restore`, () => {
        restoreCount += 1;
        return envelope({ pageId: PAGE_ID, previousVersionId: 'version-before-restore' });
      }),
    );
    renderWithProviders(<VersionList pageId={PAGE_ID} />);
    const user = userEvent.setup();

    await user.click(await screen.findByRole('button', { name: /Xem phiên bản/ }));
    await user.click(await screen.findByRole('button', { name: 'Khôi phục' }));
    expect(screen.getByText('Phiên bản hiện tại sẽ được lưu lại trước khi khôi phục.')).toBeInTheDocument();

    await user.click(screen.getByRole('button', { name: 'Huỷ' }));
    expect(restoreCount).toBe(0);

    await user.click(screen.getByRole('button', { name: 'Khôi phục' }));
    await user.click(screen.getByRole('button', { name: 'Khôi phục phiên bản' }));
