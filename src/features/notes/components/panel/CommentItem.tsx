...
function CommentBodyView({ body }: CommentBodyViewProps) {
  return (
...
export function CommentItem({ canReply, comment, currentUserId, depth, onRemove,
  ...
  const authorName = comment.author?.displayName.trim() || 'Người dùng Halo';
  ...
  return (
    <article data-comment-id={comment.id} data-comment-depth={depth} className={cn('flex min-w-0 gap-2.5', depth === 1 && 'ms-9')}>
      <Avatar size="sm" src={comment.author?.avatarUrl ?? undefined} alt={authorName} fallback={authorName.slice(0, 2)} className="size-7 bg-accent text-muted-foreground" />
      <div className="min-w-0 flex-1">
        ...
        {isEditing ? (
          ...
          <>
            <CommentBodyView body={comment.body} />
