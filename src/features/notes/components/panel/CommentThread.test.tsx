...
function useResponses(comments: Comment[], myRole: PageRole = 'COMMENT') {
  server.use(
    http.get(`${NOTION_URL}/api/v1/pages/${PAGE_ID}`, () => envelope(buildPage(myRole))),
    http.get(`${NOTION_URL}/api/v1/pages/${PAGE_ID}/comments`, () => envelope(comments)),
  );
}
...
describe('luồng bình luận của trang', () => {
  it('hiện đúng hai skeleton bubble khi đang tải', () => {
    server.use(
      http.get(`${NOTION_URL}/api/v1/pages/${PAGE_ID}`, () => envelope(buildPage('COMMENT'))),
      http.get(`${NOTION_URL}/api/v1/pages/${PAGE_ID}/comments`, () => new Promise(() => undefined)),
    );

    renderWithProviders(<CommentThread pageId={PAGE_ID} />);

    expect(screen.getByTestId('comment-thread-loading').children).toHaveLength(2);
  });
  ...
  it('hiện lỗi nhỏ và thử tải lại được', async () => {
    ...
    server.use(
      http.get(`${NOTION_URL}/api/v1/pages/${PAGE_ID}/comments`, () => envelope([])),
    );
  ...
  it('hiện trạng thái rỗng và ô nhập với quyền COMMENT', async () => {
    ...
    expect(await screen.findByText('Chưa có bình luận')).toBeInTheDocument();
    expect(screen.getByRole('textbox', { name: 'Viết bình luận…' })).toBeInTheDocument();
  });
  ...
  it('hiện dữ liệu và không cho sửa hoặc xoá bình luận của người khác', async () => {
    ...
    expect(screen.getByText('@user-1')).toHaveClass('bg-primary/10', 'text-primary');
    expect(screen.queryByRole('button', { name: 'Sửa bình luận' })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Xoá bình luận' })).not.toBeInTheDocument();
  });
  ...
  it('dồn trả lời của trả lời về đúng một cấp hiển thị', async () => {
    ...
    const { container } = renderWithProviders(<CommentThread pageId={PAGE_ID} />);
    await screen.findAllByText('Người viết');

    expect(container.querySelectorAll('[data-comment-depth="0"]')).toHaveLength(1);
    expect(container.querySelectorAll('[data-comment-depth="1"]')).toHaveLength(2);
  });
  ...
  it('gửi body segment mention đúng userId theo hợp đồng BE', async () => {
    ...
    server.use(
      http.get(`${CHAT_URL}/api/v1/users/search`, () => envelope({
        items: [{
          id: 'user-2', username: 'an', displayName: 'An', avatarUrl: null,
          friendship: 'NONE', mutualFriendsCount: 0,
        }],
        nextCursor: null,
      })),
      http.post(`${NOTION_URL}/api/v1/pages/${PAGE_ID}/comments`, async ({ request }) => {
        received = await request.json();
        return envelope(buildComment({ id: 'comment-created' }));
      }),
    );
    renderWithProviders(<CommentThread pageId={PAGE_ID} />);
    const composer = await screen.findByRole('textbox', { name: 'Viết bình luận…' });
