...
vi.mock('./InternalPermissionSection', () => ({
  InternalPermissionSection: () => <div data-testid="internal-permission-section" />,
}));
...
describe('tab Chia sẻ — cổng theo myRole', () => {
  it('hiện skeleton khi đang tải thông tin trang', () => {
    server.use(http.get(`${NOTION_URL}/api/v1/pages/${PAGE_ID}`, () => new Promise(() => undefined)));

    renderWithProviders(<ShareTab pageId={PAGE_ID} workspaceId={WORKSPACE_ID} />);
  ...
  it('hiện lỗi nhỏ và cho thử tải lại', async () => {
    ...
    renderWithProviders(<ShareTab pageId={PAGE_ID} workspaceId={WORKSPACE_ID} />);
    expect(await screen.findByText('Không tải được thông tin trang')).toBeInTheDocument();

    usePageResponse('FULL');
    server.use(
      http.get(`${NOTION_URL}/api/v1/workspaces/${WORKSPACE_ID}/members`, () => envelope([])),
      http.get(`${NOTION_URL}/api/v1/pages/${PAGE_ID}/permissions`, () => envelope([])),
    );
    await userEvent.click(screen.getByRole('button', { name: 'Thử lại' }));

    expect(await screen.findByTestId('internal-permission-section')).toBeInTheDocument();
  });
  ...
  it.each<PageRole>(['EDIT', 'COMMENT', 'VIEW'])(
    ...
    async (role) => {
      ...
      expect(await screen.findByText('Chỉ người có toàn quyền mới quản lý chia sẻ')).toBeInTheDocument();
      expect(screen.queryByTestId('internal-permission-section')).not.toBeInTheDocument();
      expect(screen.queryByTestId('public-share-section')).not.toBeInTheDocument();
    },
  );
