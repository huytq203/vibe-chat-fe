...
describe("IssueTokenDialog", () => {
  ...
  it("hiện đủ checkbox scope Bot API", () => {
    renderDialog();
    expect(
      screen.getByRole("checkbox", { name: /gửi tin nhắn/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("checkbox", { name: /quản lý tin nhắn/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("checkbox", { name: /gửi media/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("checkbox", { name: /gửi broadcast/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("checkbox", { name: /quản lý webhook/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("checkbox", { name: /quản lý command/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("checkbox", { name: /trả lời callback/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("checkbox", { name: /trả lời inline/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("checkbox", { name: /đọc analytics/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("checkbox", { name: /quản trị nhóm/i }),
    ).toBeInTheDocument();
  });
  ...
  it("tick 1 scope rồi submit → gọi botTokensApi.issue với đúng scope + gọi onIssued", async () => {
    ...
    expect(mockIssue).toHaveBeenCalledWith("bot-1", {
      scopes: ["messages:send"],
    });
    await vi.waitFor(() =>
      expect(onIssued).toHaveBeenCalledWith("bot-1:newtoken"),
    );
  });
  ...
  it("lỗi khi submit → không gọi onIssued, hiện toast lỗi", async () => {
    ...
    mockIssue.mockRejectedValue(
      new ApiError(
        500,
        "TOKEN_ISSUE_FAILED",
        "Cấp token thất bại. Thử lại sau.",
      ),
    );
    const { onIssued } = renderDialog();
